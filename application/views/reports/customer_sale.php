   ...
   /********search***************/	
	function SearchList(){
		...
		$.ajax({
			...
			data:{customers:customers,from:from,to:to},
			success: function(data){
				$('#result').html(data);
				$('#example1').DataTable({
				"footerCallback": function ( row, data, start, end, display ) {
				...
				searchfields(customers,from,to);
			}
		});
	}
	
	function setEvents(){
		$('#customers,#from,#to').change(SearchList);
	}
	function searchfields(customer,from,to){
		...
		selectopt+="<select name='customers' id='customers' class='form-control'>";
		selectopt+="<option value=''>--Select Customer--</option>";
		var allcustomers='<?php echo json_encode($customers); ?>';
		allcustomers=JSON.parse(allcustomers);
		$.each(allcustomers, function(i, data) {
			//alert(states.id);
			selectopt+='<option value="'+data.cust_id+'">'+data.cust_name+'</option>';
		});
