   ...
   /********search***************/	
	function SearchList(){
		...
		$.ajax({
			...
			data:{users:users,from:from,to:to},
			success: function(data){
				$('#result').html(data);
				$('#example1').DataTable({
				"footerCallback": function ( row, data, start, end, display ) {
				...
				searchfields(users,from,to);
			}
		});
	}
	
	function setEvents(){
		$('#users,#from,#to').change(SearchList);
	}
	function searchfields(user,from,to){
		...
		selectopt+="<select name='users' id='users' class='form-control'>";
		selectopt+="<option value=''>--Select User--</option>";
		var allusers='<?php echo json_encode($users); ?>';
		allusers=JSON.parse(allusers);
		$.each(allusers, function(i, data) {
			//alert(states.id);
			selectopt+='<option value="'+data.id+'">'+data.name+'</option>';
		});
